// 一个简单的前向 PBR 管线的实现
// 通过这个您可以一窥起源 2 引擎是怎么实现 PBR 渲染的，它的原理和这个差不多
...
{
    float fCosA = dot( vNormal, vLightDir ); // 光源和法向量夹角的cos值的计算，非常重要。因为两个向量都是单位向量，模相乘得1，我们这里直接用它们的数量积来表示cos值
...
inline float BlinnPhongNDF( in float3 halfDir, in float3 normal, in float roughnessSq ) 
{
    ...
    return pow( max( dot( halfDir, normal ), 0.0f ), power ) ;
}
...
inline float BeckmannNDF( in float3 halfDir, in float3 normal, in float roughnessSq )
{
    ...
    return exp( ( alphaSq - 1.0f ) / ( alphaSq * rsq ) ) ;
}
...
// 法线分布函数在这里计算微表面法线分布，用来做 specular，注意只是一个近似
...
float4 TexPS( PS_IN i ) : SV_Target
{
    ...
    float fresnel = SchlickFresnel( LoH ); //菲涅尔项
